...
const moveToPlanList = () => router.push(`/myplan/list`);
const moveToPlanWrite = () => router.push(`../../attraction/page`);
...
<template>
  <div class="container-md py-5">
    ...
    <div
      class="row d-flex justify-content-center rounded-0 shadow bg-white pt-4 px-4 mb-3">
      ...
      <div class="text-black-50 fst-italic ms-auto">
        ...
        >
        <small class="d-block text-end"
          >작성일:&nbsp;{{ board?.createdDate }}</small
        >
        <small class="d-block text-end"
          >수정일:&nbsp;{{ board?.updatedDate }}</small
        >
      </div>
      <div
        class="d-flex justify-content-center border-bottom border-1 border-black-50">
                ...
                <a-timeline
                  ...
                  <a-timeline-item color="green">
                    ...
                    <div class = "move" v-if="attraction.bicycle">
                      ...
                      <i class="bi bi-person-fill"></i> 도보 {{ attraction.walk }}분
                      <i class="bi bi-bicycle" style="color:red"></i> 자전거 {{ attraction.bicycle }}분
                      <i class="bi bi-car-front-fill" style="color:blue"></i> 자동차 {{ attraction.car }}분
                    </div>
      ...
      <div class="d-flex justify-content-center pb-3">
        <button @click="moveToPlanList" class="me-1 btn btn-outline-dark">
          목록으로
        </button>
        <button @click="moveToPlanWrite" class="ms-1 me-auto btn btn-dark">
          새글작성
        </button>
        <button @click="deletePlan" class="me-1 ms-auto btn btn-danger">
          삭제하기
        </button>
      </div>
    </div>
