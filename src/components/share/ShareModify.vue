...
const update = async () => {
  await axios.put(`http://localhost:8080/enjoytrip/board/update`, board.value)
  .then(response => console.log(response));
}
	...
	<div class="container-md py-5 text-white-emphasis vh-100">
		...
		<div
           ...
           type="text" name="title" id="title" class="form-control"
						placeholder="제목" v-model="board.title">
				</div>
				<div class="mb-3">
					<label for="content" class="form-label">내용</label>
					<textarea name="content" id="content" class="form-control"
						style="height: 40vh;" placeholder="내용" v-model="board.content">{{ board.content }}</textarea>
				</div>

			</form>
			<div class="d-grid gap-2">
				<button id="btn-board-modify" class="btn btn-dark" @click="update">수정하기</button>
				<button id="btn-board-delete" class="btn btn-dark" @click="deleleShare ">삭제하기</button>
			</div>
		</div>
	</div>
</div>
