...
const styleType = reactive({username:"error-message",password:"error-message",passwordCheck:"error-message",nickname:"error-message",email:"error-message",})
const success = reactive({username:false,password:false,passwordCheck:false,nickname:false,email:false})
var regExp = /[ \{\}\[\]\/?.,;:|\)*~`!^\-_+┼<>@\#$%&\'\"\\\(\=]/gi;
const vaildUserName=()=>{
		...
		<form @submit.prevent id="form-join" method="POST" action="">
			...
			<div class="col-md-6 form-floating m-3">
				<input type="text" class="form-control" id="id"
					placeholder="Enter email" name="id" v-model="user.username" 
                    @blur="vaildUserName"/> <label for="id">ID <font-awesome-icon :icon="['fas', 'check']" size="lg" style="color: #2bab3a;" v-if="success.username" /></label>
			</div>
			...
			<div class="form-floating m-3 d-flex justify-content-center">
				<button type="button" id="btn-join"
					class="btn btn-outline-success m-3" @click="signUp">회원가입</button>
